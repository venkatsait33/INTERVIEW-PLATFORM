...
import { InterviewCard, LoadingPage, StatCard, EmptyState } from '../../components/common';
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">Manage your interview sessions</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard title="Active Now" value={active.length} icon="🔴" color="green" />
        <StatCard title="Completed" value={past.length} icon="✅" color="teal" />
      </div>

      {/* Active Sessions */}
      {active.length > 0 && (
        <div>
          <h2 className="text-base font-semibold mb-4 text-red-600 flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {active.map(i => (
              <InterviewCard
                ...
                interview={i}
                actions={
                  <Link to={`/room/${i._id}`} className="btn-primary text-xs py-1.5">
                    Rejoin Room →
                  </Link>
                }
              />
      ...
      <div>
        ...
        {upcoming.length === 0 ? (
          <EmptyState icon="📅" title="No upcoming interviews" description="You're all caught up!" />
        ) : (
      ...
      {past.length > 0 && (
        <div>
          <h2 className="text-base font-semibold mb-4">Past Interviews</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
