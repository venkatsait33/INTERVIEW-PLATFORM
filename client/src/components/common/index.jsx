...
export const StatusBadge = ({ status }) => (
  <span
    className={`badge ${statusStyles[status] || "bg-gray-100 text-gray-600"}`}
  >
...
export const StatCard = ({ title, value, icon, color = "indigo", trend }) => {
  ...
  return (
    <div className="card overflow-hidden relative">
      <div
        className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br ${colors[color]} opacity-10 rounded-bl-full`}
      />
      <div className="flex items-start justify-between">
        ...
        <div
          className={`w-12 h-12 rounded-xl bg-gradient-to-br ${colors[color]} flex items-center justify-center text-2xl`}
        >
          {icon}
        </div>
      </div>
  ...
  return (
    <div className="card hover:shadow-md transition-shadow">
      ...
      <div className="space-y-1.5 text-sm text-gray-500">
        ...
        {interview.interviewer && <p>👨‍💻 {interview.interviewer.name}</p>}
        {interview.candidate && <p>👩‍💼 {interview.candidate.name}</p>}
